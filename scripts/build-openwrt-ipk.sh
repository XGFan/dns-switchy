#!/usr/bin/env bash
#
# build-openwrt-ipk.sh — 构建 dns-switchy 的两个 OpenWrt ipk
#
# 用法:
#   ./scripts/build-openwrt-ipk.sh --version v1.0.0
#   ./scripts/build-openwrt-ipk.sh --version v1.0.0 --package dns-switchy
#   ./scripts/build-openwrt-ipk.sh --version v1.0.0 --prepare-only
#
# 输出:
#   dist/ipk/<version>/artifacts/dns-switchy_<v>_<arch>.ipk
#   dist/ipk/<version>/artifacts/luci-app-dns-switchy_<v>_all.ipk
#
...
选项:
  ...
  -p, --package NAME       仅构建指定包: dns-switchy | luci-app-dns-switchy | all (默认 all)
  -a, --arch ARCH          service ipk 的架构: aarch64_generic | x86_64 | ... (默认 aarch64_generic)
  -o, --output-dir DIR     输出根目录 (默认: dist/ipk/)
      --prepare-only       仅校验 skeleton 并准备 deterministic 输出根目录
  -h, --help               显示帮助信息

环境变量:
  OPENWRT_PACKAGE_ARCH     service ipk 的架构 (--arch 优先；默认 aarch64_generic)
...
示例:
  ...
  $(basename "$0") --version v1.0.0 --package dns-switchy
  $(basename "$0") --version v1.0.0 --arch x86_64
EOF
  exit 0
}
...
selected_packages() {
  case "${PACKAGE}" in
    all)
      printf '%s\n' "${ALL_PACKAGES[@]}"
      ;;
    dns-switchy|luci-app-dns-switchy)
      printf '%s\n' "${PACKAGE}"
      ;;
    *)
...
ensure_package_skeleton() {
  ...
  case "${package}" in
    dns-switchy)
      require_file "${OPENWRT_DIR}/dns-switchy/Makefile"
      require_file "${OPENWRT_DIR}/dns-switchy/files/postinst"
      require_file "${OPENWRT_DIR}/dns-switchy/files/prerm"
      require_file "${OPENWRT_DIR}/dns-switchy/files/postrm"
      require_file "${OPENWRT_DIR}/dns-switchy/files/conffiles"
      require_file "${REPO_ROOT}/files/etc/init.d/dns-switchy"
      require_file "${REPO_ROOT}/files/etc/dns-switchy/config.yaml"
      require_file "${REPO_ROOT}/files/etc/config/dns-switchy"
      ;;
    luci-app-dns-switchy)
      require_file "${OPENWRT_DIR}/luci-app-dns-switchy/Makefile"
      require_file "${OPENWRT_DIR}/luci-app-dns-switchy/root/usr/share/luci/menu.d/dns-switchy.json"
      require_file "${OPENWRT_DIR}/luci-app-dns-switchy/root/usr/share/rpcd/acl.d/luci-app-dns-switchy.json"
      require_file "${OPENWRT_DIR}/luci-app-dns-switchy/htdocs/luci-static/resources/view/dns-switchy/index.js"
      ;;
    *)
...
build_service_ipk_locally() {
  ...
  local artifact_name="dns-switchy_${clean_version}_${package_arch}.ipk"
  ...
  local binary_path="${data_root}/usr/bin/dns-switchy"
  ...
  cp "${files_root}/etc/init.d/dns-switchy" "${data_root}/etc/init.d/dns-switchy"
  chmod 0755 "${data_root}/etc/init.d/dns-switchy"

  # UCI defaults (conffile, preserved on upgrade)
  mkdir -p "${data_root}/etc/config"
  cp "${files_root}/etc/config/dns-switchy" "${data_root}/etc/config/dns-switchy"
  chmod 0644 "${data_root}/etc/config/dns-switchy"

  # Default yaml config (conffile, preserved on upgrade)
  mkdir -p "${data_root}/etc/dns-switchy"
  cp "${files_root}/etc/dns-switchy/config.yaml" "${data_root}/etc/dns-switchy/config.yaml"
  chmod 0644 "${data_root}/etc/dns-switchy/config.yaml"

  # Control scripts
  cp "${OPENWRT_DIR}/dns-switchy/files/postinst" "${control_root}/postinst"
  cp "${OPENWRT_DIR}/dns-switchy/files/prerm" "${control_root}/prerm"
  cp "${OPENWRT_DIR}/dns-switchy/files/postrm" "${control_root}/postrm"
  cp "${OPENWRT_DIR}/dns-switchy/files/conffiles" "${control_root}/conffiles"
...
Maintainer: dns-switchy project
License: MIT
Section: net
Priority: optional
Installed-Size: ${installed_size}
Description: Rule-based DNS proxy with multi-upstream forwarding, caching and embedded web portal
EOF
...
build_luci_shell_ipk_locally() {
  ...
  local package_version="${clean_version}-1"
  local artifact_dir="${output_root}/artifacts"
  local artifact_name="luci-app-dns-switchy_${clean_version}_${package_arch}.ipk"
  local artifact_path="${artifact_dir}/${artifact_name}"
  local workspace
  workspace="$(mktemp -d "${output_root}/build.XXXXXX")"
  local data_root="${workspace}/data"
  ...
  mkdir -p \
    "${artifact_dir}" \
    "${data_root}/www/luci-static/resources/view/dns-switchy" \
    "${data_root}/usr/share/luci/menu.d" \
    "${data_root}/usr/share/rpcd/acl.d" \
    "${control_root}"

  cp "${OPENWRT_DIR}/luci-app-dns-switchy/htdocs/luci-static/resources/view/dns-switchy/index.js" \
    "${data_root}/www/luci-static/resources/view/dns-switchy/index.js"
  cp "${OPENWRT_DIR}/luci-app-dns-switchy/root/usr/share/luci/menu.d/dns-switchy.json" \
    "${data_root}/usr/share/luci/menu.d/dns-switchy.json"
  cp "${OPENWRT_DIR}/luci-app-dns-switchy/root/usr/share/rpcd/acl.d/luci-app-dns-switchy.json" \
    "${data_root}/usr/share/rpcd/acl.d/luci-app-dns-switchy.json"
  chmod 0644 \
    "${data_root}/www/luci-static/resources/view/dns-switchy/index.js" \
    "${data_root}/usr/share/luci/menu.d/dns-switchy.json" \
    "${data_root}/usr/share/rpcd/acl.d/luci-app-dns-switchy.json"
...
Installed-Size: ${installed_size}
Description: DNS-Switchy LuCI shell entry package
EOF
...
main() {
  ...
  echo "版本: ${VERSION}"
  echo "包: ${packages[*]}"
  echo "服务包架构: ${DEFAULT_PACKAGE_ARCH}"
