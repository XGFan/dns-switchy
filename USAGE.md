...
**部署约束**：因为要绑定 5353，同机不能运行 avahi / umdns 等 mDNS 应答方；本 resolver 只应部署在无 mDNS 栈的机器上（典型：OpenWrt 路由器）。**链条顺序**：mdns 必须排在任何 accept-all（无 `rule`）resolver 之前——链条是第一个 `Accept` 的 resolver 处理查询，若 `.local` 被排在前面的兜底 resolver 抢走，内网主机名会泄漏到公网上游。另外 macOS / iOS 等自带 mDNS 的客户端解析 `.local` 从不询问 DNS 服务器，本 resolver 服务不到它们，也不需要。
